import { pgTable, uuid, varchar, integer, timestamp, boolean, text, pgEnum } from 'drizzle-orm/pg-core';
import { user } from './auth';
import { projects } from './projects';
import { issues } from './issues';

export const deletionTypeEnum = pgEnum('deletion_type', ['self', 'admin_override']);
export const auditActionEnum = pgEnum('audit_action', ['self_delete', 'admin_override_delete', 'admin_override_mark_unpaid']);
export const activityLevelEnum = pgEnum('activity_level', ['none', 'low', 'medium', 'high']);
